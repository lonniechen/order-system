...
export class CoordinateValidationPipe implements PipeTransform {
    async transform(coordinate: any, metaData: ArgumentMetadata) {
        ...
        if (!coordinate) {
            throw new BadRequestException(`${metaData.data} is required in the request body`)
        }
        ...
        coordinate.forEach((element: any) => {
            if (typeof element !== 'string') {
                throw new BadRequestException(`${metaData.data} should be an array of 2 string elements in the reqeust body`)
            }
        })
